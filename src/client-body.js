 ...
 * 经 ctx.remote.settings.mutate（Remote 命名空间，位置参数 ns/ops/expectedRevision）
 * 带冲突检测；写入后 pi-ai 适配器实时重挂载，无需重启 web。
 */
...
function CapsCard(props) {
  ...
  async function save() {
    ...
    try {
      ...
      // 写走 ctx.remote.settings.mutate（位置参数 + {ok,error} 应答）；
      // connection 服务只有 rpc/reconnect/generation，无 .api 成员。
      var response = await settings.mutate(
        NS,
        [{ op: "set", path: ["providers", route], value: draft }],
        props.freshRevision(),
      )
      if (!response.ok) {
        ...
        setError(failure ? failure.message : "写入失败")
        return
      }
      setSavedAt(Date.now())
      await props.reload()
...
var inject = ["slots", "remote", "remote.settings", "settingsScope"]

function apply(ctx) {
  var store = createCapsStore(ctx)
  store.reload()
  // 写走 ctx.remote.settings（Remote 命名空间）；connection 服务不提供 .api。
  var settings = ctx.remote.settings

  // 页面启动即按已存偏好应用 Think 行显隐（无需先打开本面板）。
  applyHideStyle(readHidePref())

  function ModelCapsSection(propsFromSlot) {
    return h(CapsPanel, { store: store, settings: settings, close: propsFromSlot && propsFromSlot.close })
  }
