...
function patchEntry(entry, patch) {
	// 只在字段显式出现时才应用：缺省=保持原值，null/[]=删除，false=非思考模型。
	if ('input' in patch) {
...
export function apply(ctx) {
	...
	const setCaps = async (args) => {
		...
		const patch = { model: args.model, override: args.override === true };
		// 只在字段显式给出时才写入：缺省=保持原值，仅 null/[]/false 触发删除/覆盖。
		if ('input' in args) patch.input = args.input;
		if ('efforts' in args) patch.efforts = args.efforts;
		if ('defaultEffort' in args) patch.defaultEffort = args.defaultEffort;
		const profile = applyCapsPatch(user ?? {}, patch);
